...
Options:
  ...
  --workspace-version VERSION
                        Set codex-rs workspace.package.version after applying
                        the patch series and before generating or compiling.
...
require_repo_root() {
  ...
  [ -f "${REPO_ROOT}/scripts/release/codex_version.py" ] || die "missing scripts/release/codex_version.py"
...
apply_release_patches() {
  ...
  series_output="$(create_release_scratch_file "reconcile-series")"
  if ! python3 "${worktree_dir}/scripts/release/release_contract.py" \
      print-patch-series \
      --patches-dir "${worktree_dir}/${PATCH_RELEASE_DIR}" \
      --series-file "${worktree_dir}/${PATCH_SERIES_FILE}" >"${series_output}"; then
    rm -f -- "${series_output}"
    return 1
  fi
  mapfile -t patches <"${series_output}"
  rm -f -- "${series_output}"
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    --worktree-dir)
      ...
      ;;
    --workspace-version)
      [ $# -ge 2 ] || die "--workspace-version requires a value"
      WORKSPACE_VERSION="$2"
      shift 2
      ;;
    --keep-worktree)
...
if [ -n "${WORKSPACE_VERSION}" ]; then
  info "Setting workspace package version to ${WORKSPACE_VERSION}"
  run python3 "${WORKTREE_DIR}/scripts/release/codex_version.py" \
    set "${WORKSPACE_VERSION}"
fi
